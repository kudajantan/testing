...
public class NgnNetworkService  extends NgnBaseService implements INgnNetworkService {
	...
	private boolean requestCellularRouteToHostApi8(String host, boolean IPv6)
	{
		...
		try {
			Method method = connectivityManager.getClass().getMethod("startUsingNetworkFeature", int.class, String.class);
			resultInt = (int) method.invoke(connectivityManager, ConnectivityManager.TYPE_MOBILE, "enableHIPRI");
	...
	private boolean bindProcessToConnectionApi8(final NgnNetworkConnection connection) {
		...
		try{
			Method method = connectivityManager.getClass().getMethod("requestRouteToHost", int.class, int.class);
			resultBool = (boolean) method.invoke(connectivityManager, ConnectivityManager.TYPE_MOBILE_HIPRI, hostAddressInt);
		} catch (Exception e) {
			//Do something
		}
